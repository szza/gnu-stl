...
namespace tr1
{
  ...
  template<typename _Key, typename _Value, typename _Allocator,
    ...
    : public __detail::_Rehash_base<_RehashPolicy,
      ...
      public __detail::_Map_base<_Key, _Value, 
                                 _ExtractKey,
                                 __unique_keys,
                                 _Hashtable<_Key, _Value, _Allocator,
                                            _ExtractKey,
  ...
  /// @brief 从 __p 处开始寻找是否存在键是 __k 的节点，存在则返回，不存在则返回NULL   
  template<typename _Key, typename _Value,
	   typename _Allocator, typename _ExtractKey, typename _Equal,
	   typename _H1, typename _H2, typename _Hash, typename _RehashPolicy,
	   bool __chc, bool __cit, bool __uk>
    typename _Hashtable<_Key, _Value, _Allocator, _ExtractKey, _Equal, _H1, _H2, _Hash, _RehashPolicy, __chc, __cit, __uk>::_Node*
             _Hashtable<_Key, _Value, _Allocator, _ExtractKey, _Equal, _H1, _H2, _Hash, _RehashPolicy, __chc, __cit, __uk>::
    _M_find_node(_Node* __p, const key_type& __k, typename _Hashtable::_Hash_code_type __code) const
    {
        for (; __p; __p = __p->_M_next)
            // 如果 __p 的hashcode 和 __code 相等
            // __p 的值和 __k 也相等
            // 则返回 __p
            if (this->_M_compare(__k, __code, __p))
                return __p;
  ...
  template<typename _Key, typename _Value,
	   ...
	   bool __chc, bool __cit, bool __uk>
    // 下面的返回类型，iterator
    typename _Hashtable<_Key, _Value, _Allocator, _ExtractKey, _Equal, _H1, _H2, _Hash, _RehashPolicy, __chc, __cit, __uk>::iterator
             _Hashtable<_Key, _Value, _Allocator, _ExtractKey, _Equal, _H1, _H2, _Hash, _RehashPolicy, __chc, __cit, __uk>:: // 这一行是类
    _M_insert(const value_type& __v, std::tr1::false_type) // 函数名
    {
        ...
        const key_type& __k = this->_M_extract(__v);                           // 构造键 _k
        typename _Hashtable::_Hash_code_type __code = this->_M_hash_code(__k); // 计算键的 hashcode
        size_type __n = this->_M_bucket_index(__k, __code, _M_bucket_count);   // 计算键的 index

        // First find the node, avoid leaking new_node if compare throws.
        // 从 _M_buckets[__n] 桶中查找，是否存在这个节点 _k
        _Node* __prev = _M_find_node(_M_buckets[__n], __k, __code);   // 待插入的桶的位置
        _Node* __new_node = _M_allocate_node(__v);                    // 为新节点分配内存
  ...
  template<typename _Key, typename _Value,
      ...
      __try
    {
	  for (size_type __i = 0; __i < _M_bucket_count; ++__i)
        // 将原来的每个桶都插入到新的 __new_array 只能
	    while (_Node* __p = _M_buckets[__i]) {
            std::size_t __new_index = this->_M_bucket_index(__p, __n); // 1 节点 __p 在新的桶中的index
            _M_buckets[__i] = __p->_M_next;                            // 2 采用头部插入法
            __p->_M_next = __new_array[__new_index];
            __new_array[__new_index] = __p;
        }
        // 已完全迁移到 __new_array 中 
        // 就需要释放原来的内存
        _M_deallocate_buckets(_M_buckets, _M_bucket_count);
        // 更新
        _M_bucket_count = __n;
