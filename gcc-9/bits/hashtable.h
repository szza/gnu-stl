   ...
   */
    ...
    class _Hashtable
        : public __detail::_Hashtable_base<_Key, _Value, _ExtractKey, _Equal, _H1, _H2, _Hash, _Traits>,
          ...
          private __detail::_Hashtable_alloc<__alloc_rebind<_Alloc,
        ...
        ->_Hashtable &
    {
        ...
        _M_assign_elements(__ht,
                           [](const __reuse_or_alloc_node_type &__roan, const __node_type *__n) { return __roan(__n->_M_v()); });
        return *this;
    }
        ...
        _M_assign_elements(_Ht && __ht, const _NodeGenerator &__node_gen)
    {
        ...
        __try
        {
            ...
            _M_before_begin._M_nxt = nullptr;
            _M_assign(__ht,
                      [&__node_gen, &__roan](__node_type *__n) { return __node_gen(__roan, __n); });
            if (__former_buckets)
            ...
            _M_move_assign(_Hashtable && __ht, std::false_type)
    {
        ...
        else
        {
            ...
            _M_assign_elements(std::move(__ht),
                               [](const __reuse_or_alloc_node_type &__roan, __node_type *__n) { return __roan(std::move_if_noexcept(__n->_M_v())); });
            __ht.clear();
        }
    }
    ...
    _Hashtable<_Key, _Value, _Alloc, _ExtractKey, _Equal,
    _H1, _H2, _Hash, _RehashPolicy, _Traits>::
    _M_find_before_node(size_type __n, const key_type &__k, __hash_code __code) const ->__node_base *
    {
        __node_base *__prev_p = _M_buckets[__n]; // 哨兵
        // 这是个空桶，则直接return
        if (!__prev_p)
            return nullptr;
    ...
    _M_insert_bucket_begin(size_type __bkt, __node_type * __node)
    {
        if (_M_buckets[__bkt]) {
            ...
            __node->_M_nxt = _M_before_begin._M_nxt;    // 和前一个桶之间建立联系
            _M_before_begin._M_nxt = __node;
            if (__node->_M_nxt)
                // __node 不是第一个桶中的第一个节点
                // We must update former begin bucket that is pointing to
                // _M_before_begin.
                // 前一个桶的哨兵节点，设置为 __node
                _M_buckets[_M_bucket_index(__node->_M_next())] = __node;
            // 将新的桶的哨兵设置为 _M_before_begin
    ...
    auto
    _Hashtable<_Key, _Value, _Alloc, _ExtractKey, _Equal, _H1, _H2, _Hash, _RehashPolicy, _Traits>::
    _M_emplace(std::true_type, _Args && ...__args) ->pair<iterator, bool>
    {
        // First build the node to get access to the hash code
        // 分配节点
        __node_type *__node = this->_M_allocate_node(std::forward<_Args>(__args)...);
        // 取出节点的值
        const key_type &__k = this->_M_extract()(__node->_M_v());   // 键
        __hash_code __code;                                         // 键对应的 hashcode
        __try
        {
        ...
        size_type __bkt = _M_bucket_index(__k, __code);     // index
    ...
    auto
    _Hashtable<_Key, _Value, _Alloc, _ExtractKey, _Equal,_H1, _H2, _Hash, _RehashPolicy, _Traits>::
    _M_emplace(const_iterator __hint, std::false_type, _Args && ...__args) ->iterator
    {
    ...
    auto
    _Hashtable<_Key, _Value, _Alloc, _ExtractKey, _Equal, _H1, _H2, _Hash, _RehashPolicy, _Traits>::
    _M_insert_unique_node(size_type __bkt, __hash_code __code, __node_type * __node, size_type __n_elt)->iterator
    {
        const __rehash_state &__saved_state = _M_rehash_policy._M_state();
        std::pair<bool, std::size_t> __do_rehash = _M_rehash_policy._M_need_rehash(_M_bucket_count,
                                                                                    _M_element_count,
                                                                                    __n_elt);
        __try
        {
            ...
            if (__do_rehash.first) {
                ...
                __bkt = _M_bucket_index(this->_M_extract()(__node->_M_v()), __code); // 当前待插入节点在新桶中的索引
            }

            this->_M_store_code(__node, __code); // 将 __node 的hashcode存储在 __node 中：
    ...
    {
        const __rehash_state &__saved_state = _M_rehash_policy._M_state();
        std::size_t __buckets = std::max(_M_rehash_policy._M_bkt_for_elements(_M_element_count + 1),
                                         __n);
        ...
        _M_rehash_aux(size_type __n, std::true_type)
    {
        ...
        __node_type *__p = _M_begin();                           // 旧桶的首节点
        _M_before_begin._M_nxt = nullptr;
        ...
        while (__p)
        {
            ...
            std::size_t __bkt = __hash_code_base::_M_bucket_index(__p, __n); // 节点 p 在新桶中的索引
            // __new_buckets[__bkt] 不存在的
            if (!__new_buckets[__bkt]) // 没有链表
            {
                // 使得 __new_buckets[__bkt]的首届点指向前一个桶的首节点，
                // 如果没有前一个桶，则指向NULL
                // 注意： 由于采用的是头插法，因此在完成插入后，p会变成尾部节点
                // 因此，最后情况会变成一个桶的尾部节点指向另一个桶的首节点
                __p->_M_nxt = _M_before_begin._M_nxt; // 这是为了在各个桶之间建立连接
                                                      //  使得 _M_before_begin 指向新桶的首节点
                _M_before_begin._M_nxt = __p;
                // _M_before_begin 作为新桶的哨兵
                __new_buckets[__bkt] = &_M_before_begin;

                // __bbegin_bkt 是上一个桶的索引，
                // __new_buckets[__bbegin_bkt] 则是第一个桶的哨兵，即 __p
                // 那么 __p->_M_nxt 就上一个桶的第一个节点，
                // 也因此，符合 __p->_M_nxt = _M_before_begin._M_nxt;
                if (__p->_M_nxt)
        ...
        /// 完成后， @c _M_before_begin->_M_next 指向的就是头部节点
    ...
    {
        __bucket_type *__new_buckets = _M_allocate_buckets(__n); // 分配 __n 个桶
        ...
        {
            __node_type *__next = __p->_M_next();
            std::size_t __bkt = __hash_code_base::_M_bucket_index(__p, __n); // 在新桶中的索引
