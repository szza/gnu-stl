...
namespace std _GLIBCXX_VISIBILITY(default)
{
   ...
   *  unordered_map, false for unordered_multiset and unordered_multimap.
   */
   ...
   *  template parameter of class template _Hashtable controls w  hether
   ...
   */
        ...
        struct _Power2_rehash_policy
        {
            ...
            _M_next_bkt(std::size_t __n) noexcept
            {
                ...
                const auto __max_bkt   = size_t(1) << (__max_width * __CHAR_BIT__ - 1); // 最大的桶数 : 2^(63)
                std::size_t __res      = __clp2(__n);  // 计算不小于2的幂

                if (__res == __n)   
                    __res <<= 1;        // 翻倍

                if (__res == 0)         // 如果是0，则直接分配最大
                    __res = __max_bkt;  
            ...
            // is the new bucket count.  If not, return make_pair(false, 0).
            /// @brief  检测是否需要rehash
            /// @param  @c __n_bkt 目前桶的个数
            /// @param  @c __n_elt 目前总的元素个数
            /// @param  @c __n_ins 此次插入的元素个数
            /// @return 需要rehash，返回 std::pair{true, n}，n 是新的桶个数
            ///         否则返回 std::pair{false, 0}
            std::pair<bool, std::size_t>
                ...
                if (__n_elt + __n_ins >= _M_next_resize) 
                {
                    ...
                    if (__min_bkts >= __n_bkt)
                        return std::make_pair(true,
                                              _M_next_bkt(std::max<std::size_t>(__builtin_floor(__min_bkts) + 1,  // 向下取整+1
                                                                                __n_bkt * _S_growth_factor)));    // 倍增
   ...
   */
        ...
        {
        private:
            using __hashtable_base = __detail::_Hashtable_base<_Key, 
                                                               _Pair,
                                                               _Select1st,
                                                               _Equal, 
                                                               _H1, 
            ...
            using __hashtable = _Hashtable<_Key, 
                                           _Pair, 
                                           _Alloc,
                                           _Select1st,
                                           _Equal,
        ...
        template <typename _Key, 
                  ...
                  typename _RehashPolicy, 
                  typename _Traits>
        auto _Map_base<_Key, _Pair, _Alloc, _Select1st, _Equal, _H1, _H2, _Hash, _RehashPolicy, _Traits, true>::
        operator[](const key_type &__k) -> mapped_type &
        {
        ...
        operator[](key_type &&__k) -> mapped_type&
        {
            __hashtable *__h = static_cast<__hashtable *>(this);
            __hash_code __code = __h->_M_hash_code(__k);
            std::size_t __n = __h->_M_bucket_index(__k, __code);
        ...
        auto
        _Map_base<_Key, _Pair, _Alloc, _Select1st, _Equal, _H1, _H2, _Hash, _RehashPolicy, _Traits, true>::
        at(const key_type &__k) -> mapped_type &
        {
            __hashtable *__h = static_cast<__hashtable *>(this);
            __hash_code __code = __h->_M_hash_code(__k);
            std::size_t __n = __h->_M_bucket_index(__k, __code);
        ...
        auto
        _Map_base<_Key, _Pair, _Alloc, _Select1st, _Equal, _H1, _H2, _Hash, _RehashPolicy, _Traits, true>::
        at(const key_type &__k) const -> const mapped_type &
        {
   ...
   */
        ...
        protected:
            ...
            //  __hashtable 继承了__detail::_Insert，而  _Insert 是 _Insert_base 的子类
            // 从父类转换为子类
            __hashtable &
            ...
            void
            _M_insert_range(_InputIterator __first, _InputIterator __last, const _NodeGetter &, true_type);

            template <typename _InputIterator, typename _NodeGetter>
            void
            _M_insert_range(_InputIterator __first, _InputIterator __last, const _NodeGetter &, false_type);
        ...
        _M_insert_range(_InputIterator __first, _InputIterator __last, const _NodeGetter &__node_gen, false_type)
        {
   ...
   */
            ...
            : public _Insert_base<_Key, _Value, _Alloc, _ExtractKey, _Equal, _H1, _H2, _Hash, _RehashPolicy, _Traits>
        {
            ...
            {
                __hashtable &__h = this->_M_conjure_hashtable(); // 将自己转为 ____hashtable 类型
  ...
  */
   ...
   */
        ...
        /// Specialization not using EBO. e
        // 不是继承关系，是has-a
        template <int _Nm, typename _Tp>
   ...
   */
        ...
        protected:
            ...
            _M_incr()
            {
                ...
                if (_M_cur)
                {
                    std::size_t __bkt = __base_type::_S_get(*this)(_M_cur->_M_hash_code,
                                                                   _M_bucket_count);
                    if (__bkt != _M_bucket)
                        _M_cur = nullptr;
                }
