...
namespace std _GLIBCXX_VISIBILITY(default)
{
  ...
  struct __allocator_traits_base
  {
    ...
    // 作用就是使用分配器_Tp为 _Up分配内存
    // 特化版本：当分配器 _Tp 有成员函数 rebind 时触发
  ...
  // __alloc_rebind 函数，是全局的 
  /// @c _Alloc 是内存分配器
  /// @c _UP 是待分配的类型
  // __alloc_rebind 是使用 _Alloc 为 _Up分配内存的分配器
  ...
  */
  ...
  struct allocator_traits : __allocator_traits_base
  {
    ...
    /// The allocated type
    typedef typename _Alloc::value_type value_type; // 需要分配内存的对象类型
  ...
  private:
    ...
    _S_construct(_Alloc &, _Tp *__p, _Args &&...__args) noexcept(noexcept(::new ((void *)__p)
    ...
    static auto
    _S_destroy(_Alloc2 &__a, _Tp *__p, int) noexcept(noexcept(__a.destroy(__p)))
    ...
    template <typename _Alloc2, typename _Tp>
    static void
    _S_destroy(_Alloc2 &, _Tp *__p, ...) noexcept(noexcept(__p->~_Tp()))
    {
      __p->~_Tp();
    }
  ...
  public:
    ...
    /**
      ...
      */
    template <typename _Tp, typename... _Args>
    static auto construct(_Alloc &__a, _Tp *__p, _Args &&...__args) noexcept(noexcept(_S_construct(__a,
                                                                                                   __p,
