...
#pragma GCC diagnostic pop
  ...
  // _Sp_counted_base 作用，线程安全地管理引用计数：
  ...
  public:
    ...
    // managed by *this.
    // 将引用计数 _M_use_count ==0 时，释放 this 管理的内存
    // 由子类实现
    virtual void _M_dispose() noexcept = 0;

    // Called when _M_weak_count drops to zero.
    // 当 _M_weak_count ==0，释放 this
    ...
    // _M_add_ref_lock 不抛出异常的版本
    bool _M_add_ref_lock_nothrow();

    // 减少 _M_use_count 、_M_weak_count 一次引用计数，
    // 如果当前 _M_use_count == 1，则析构管理的内存
    // 如果当前 _M_weak_count ==1，则析构 this
    void _M_release() noexcept
    {
      ...
      if (__gnu_cxx::__exchange_and_add_dispatch(&_M_use_count, -1) == 1)
      {
        ...
        // 在 _M_dispose() 和 _M_destroy() 之间需要一个内存屏障，
        // 确保 _M_dispose() 产生的作用在运行 _M_destroy() 的线程是可见的
        ...
        // 为 _M_weak_count 减少1
        // 如果 _M_weak_count 也是1，那么这个对象就自我析构
    ...
    long _M_get_use_count() const noexcept
    {
      ...
      // with other threads.
      return __atomic_load_n(&_M_use_count, __ATOMIC_RELAXED); // 返回 _M_use_count 的值
    }
  ...
  template <>
  inline void_Sp_counted_base<_S_atomic>::_M_add_ref_lock()
  {
    ...
    do
    {
      ...
      // 当成功， _M_use_count = __count +1，失败了则将 __count = _M_use_count
    } while (!__atomic_compare_exchange_n(&_M_use_count, 
                                          &__count, 
                                          __count + 1,
                                          true,               // weak
                                          __ATOMIC_ACQ_REL,
  ...
  // _Sp_counted_ptr 有 final 修饰，不可以继承
  ...
  private:
    //此处还有 _Sp_counted_base 的两个引用计数
    // _Sp_counted_base 负责计数
    // _M_ptr 负责资源
    _Ptr _M_ptr;  // 管理资源

  };

  // 当 _Sp_counted_ptr 当传入 nullptr 时的特化情况
  template <>
  ...
  {
    // 有些类，既包含析构器的功能，又包含了分配器的功能
    // 因此需要设置 0,1 来避免无法开启空基类优化
    // _Impl 负责 _Ptr 对象的分配、析构
  ...
  // make_shared 的计数器也是继承 _Sp_counted_base
  // 这个函数是比较高效的，因为仅仅一次分配
  template <typename _Tp, typename _Alloc, _Lock_policy _Lp>
  ...
  // 计数器也需要锁，因此模板参数设置了一个锁
  // 内部有： _Sp_counted_base
  template <_Lock_policy _Lp>
  class __shared_count
  {
    template <typename _Tp>
  ...
  public:
    constexpr __shared_count() noexcept : _M_pi(nullptr)
    { }

    template <typename _Ptr>
    explicit __shared_count(_Ptr __p) : _M_pi(nullptr)
    {
      __try
      {
        // _Sp_counted_ptr 是 _Sp_counted_base 的子类
    ...
    // 委托构造函数
    // 调用的是上面的构造函数，此时没有设置析构器，默认是delete
    template <typename _Ptr>
...
#pragma GCC diagnostic pop
    ...
    __shared_count& operator=(const __shared_count &__r) noexcept
    {
      ...
      {
        if (__tmp != nullptr)
          __tmp->_M_add_ref_copy(); // src 需要增加一个引用计数
        if (_M_pi != nullptr)
          _M_pi->_M_release();      // dst 需要释放
        // 复制
        _M_pi = __tmp;
      }
      return *this;
  ...
  private:
    friend class __weak_count<_Lp>;   // 弱引用计数

    _Sp_counted_base<_Lp>* _M_pi;    // 父类的计数器
  };

  // 内部和 _sharred_count 成员变量一致
  template <_Lock_policy _Lp>
  class __weak_count
  {
  public:
  ...
  {
    if (_M_pi != nullptr)
      _M_pi->_M_add_ref_lock(); // 又多了一个 shared_ptr
    else
      __throw_bad_weak_ptr();
  }
...
#define __cpp_lib_shared_ptr_arrays 201611L
  ...
  private:
    ...
    using _UniqCompatible 
      = typename enable_if<__and_<__sp_compatible_with<_Yp *, _Tp *>, is_convertible<_Ptr, element_type *>>::value,_Res>::type;
  ...
  protected:
    ...
    template <typename _Tp1, typename _Del,
              typename enable_if<__and_< __not_<is_array<_Tp>>, 
                                        is_array<_Tp1>, 
...
#pragma GCC diagnostic pop
  ...
  private:
    template <typename _Yp>
    using __esft_base_t 
      = decltype(__enable_shared_from_this_base(std::declval<const __shared_count<_Lp> &>(),
                                                std::declval<_Yp *>()));
