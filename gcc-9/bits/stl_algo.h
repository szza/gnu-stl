...
namespace std _GLIBCXX_VISIBILITY(default)
{
  ...
  */
  ...
  {
    std::__make_heap(__first, __middle, __comp);  // 建立堆[first, middle]，便有序
    // 
  ...
  */
  ...
  {
    typename iterator_traits<_RandomAccessIterator>::value_type __val = std::move(*__last); // 待插入的节点
    _RandomAccessIterator __next = __last; 
    --__next;   // 开始遍历的位置
    /// @brief: 已知道 @c val > @c _first 
    // __val < __next 
    while (__comp(__val, __next))
    {
      *__last = std::move(*__next); // 把 _next 位置的值放到 __last上去
      __last = __next;              // 这也 __last 就放指向了当前位置
      --__next;
    } 
                   ...
                   _Compare __comp)
  {
    ...
    // 从头部开始遍历: (fist, last] 中找到： 对于当前_i，找到它合适位置
    for (_RandomAccessIterator __i = __first + 1; __i != __last; ++__i)
    {
      // _i < _first
      // 总先是和左边的进行比较
      // 那么需要把这个 __i 处的元素放在 __first 前面
      if (__comp(__i, __first))
      {
        ...
        *__first = std::move(__val);               // 将 _i 位置处的值移动到 __first处
      }
      else
      {
        // _i >= _fist
        // 此时
        // 那么就需要在 (fist，_i)有序区间,寻找位置插入
  ...
  */
                        ...
                        _Compare __comp)
  {
    while (true)
    {
      ...
      while (__comp(__pivot, __last))
        --__last;
      if (!(__first < __last)) // __fist >= __last ，此轮结束
        return __first;       // 返回分割点
      std::iter_swap(__first, __last);  
      ++__first;
    }
  }
  ...
  // 堆排序
  // 选出最小的 ____middle - __first 个元素，放在[first, midlde)，使其按照 _Compare 比较策略进行排序
  // 其余的元素，放在 [middle, last]中
  template <typename _RandomAccessIterator, typename _Compare>
  inline void
                   ...
                   _Compare __comp)
  {
    ...
    while (__last - __first > int(_S_threshold))
    {
      if (__depth_limit == 0)
      {
        // 实际上就对[fist, last]区间进行堆排序
        // 此时的 [first, last]范围很小了
        // 小区间内变成有序的
      ...
      --__depth_limit;
      // 普通的寻找轴点的方法：
      // + _cut 左边的点都比 _cut处的值小
      // + _cut 右边的值都比 _cut处的值大 
...
#define __cpp_lib_clamp 201603
  ...
  clamp(const _Tp &__val, const _Tp &__lo, const _Tp &__hi, _Compare __comp)
  {
    ...
    return __comp(__val, __lo) ? __lo : __comp(__hi, __val) ? __hi
                                                            : __val;
  }
#endif // C++17
#endif // C++14
