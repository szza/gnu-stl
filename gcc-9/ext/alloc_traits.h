...
*/
  ...
  private:
  // _Ptr不是个指针，但是_Ptr 和 value_type* 相同，
  // __is_custom_pointer 才是 true
  ...
  public:
    ...
    construct(_Alloc &__a, _Ptr __p, _Args &&...__args) noexcept(noexcept(_Base_type::construct(__a, std::__to_address(__p),
                                                                                                std::forward<_Args>(__args)...)))
    {
      // 使用分配器 __a, 在 __p处分配内存，并调用构造函数
